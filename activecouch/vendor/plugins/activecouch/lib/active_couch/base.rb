  ...
  class Base
    ...
    class << self # Class methods
      ...
      def count(search_params = {})
        path = "/#{database_name}/_design/#{query_string(search_params[:params], {:limit => 0})}"
      ...
      private
        ...
        def find_every(search_params, overriding_options = {})
          ...
          when String
            ...
            path = "/#{database_name}/_design/#{query_string(search_params[:params], options)}"
          end
        ...
        def query_string(search_params, options)
          ...
          query_string = "by_#{key}/_view/by_#{key}?key=#{search_params[key].to_s.url_encode}"
          query_string = "#{query_string}&skip=#{options[:offset]}" unless options[:offset].nil?
          query_string = "#{query_string}&limit=#{options[:limit]}" unless options[:limit].nil?
